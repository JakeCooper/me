...
const GlobeViz = ({ regions, currentRegion, connections = [], shootingStars = [], animatingConnections = [], userLocation, connectedUsers = [] }: GlobeVizProps) => {
  ...
  const userPoints = useMemo(() => {
    // Group users by location (round to 2 decimal places for clustering)
    const locationKey = (lat: number, lng: number) =>
      `${lat.toFixed(2)},${lng.toFixed(2)}`;

    const grouped = new Map<string, { lat: number; lng: number; count: number; isCurrentUser: boolean }>();

    // Current user's location key
    const currentUserKey = userLocation ? locationKey(userLocation.lat, userLocation.lng) : null;

    // Count all connected users (this includes the current user from Redis)
    connectedUsers.forEach(user => {
      const key = locationKey(user.lat, user.lng);
    ...
    // If current user isn't in connectedUsers yet (race condition), add them
    if (userLocation && !grouped.has(currentUserKey!)) {
      grouped.set(currentUserKey!, {
    ...
    return Array.from(grouped.values()).map(point => ({
      ...
      type: point.isCurrentUser ? 'currentUser' : 'connectedUser',
      count: point.count
    }));
    ...
    <div style={{
      ...
      <ReactGlobe
        ...
        customThreeObject={d => {
          ...
          // Show count for clustered users
          if ((d.type === 'currentUser' || d.type === 'connectedUser') && d.count > 1) {
            const canvas = document.createElement('canvas');
            canvas.width = 64;
            canvas.height = 64;
            const ctx = canvas.getContext('2d')!;
            ...
            ctx.textAlign = 'center';
            ctx.textBaseline = 'middle';
            ctx.fillText(d.count.toString(), 32, 32);
        ...
        customThreeObjectUpdate={(obj, d) => {
          ...
          // Update user cluster count label
          if (label && (d.type === 'currentUser' || d.type === 'connectedUser') && d.count > 1) {
            const spriteMaterial = label.material as THREE.SpriteMaterial;
            const canvas = spriteMaterial.map!.source.data as HTMLCanvasElement;
            const ctx = canvas.getContext('2d')!;
...
export function Counter({ regions, currentRegion }: CounterProps) {
  ...
  // Derive connected users from connections (more reliable than separate state)
  const connectedUsers = React.useMemo(() =>
    connections.map(conn => ({ lat: conn.from.lat, lng: conn.from.lng })),
    [connections]
  );
      ...
      <div className={`content-column ${contentExpanded ? 'expanded' : ''}`}>
        <div style={{
          ...
          <div style={{
            ...
            <a href="https://x.com/JustJake" target="_blank" rel="noopener noreferrer" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </a>
            <a href="https://github.com/JakeCooper" target="_blank" rel="noopener noreferrer" style={{ color: 'rgba(255, 255, 255, 0.5)' }}>
              <svg width="20" height="20" viewBox="0 0 24 24" fill="currentColor">
        ...
        <p style={{ marginBottom: '1rem', lineHeight: '1.6', color: 'rgba(255, 255, 255, 0.8)' }}>
          ...
          , the smoothest way to ship software. We're remote, employing {teamCount} people around the world,
        ...
        <p style={{ marginBottom: '1rem', lineHeight: '1.6', color: 'rgba(255, 255, 255, 0.8)' }}>
          ...
          <br />
          This website included. <a href="https://github.com/jakecooper/me" style={{ color: '#E835A0', textDecoration: 'none' }}>Source here</a>.
        </p>
